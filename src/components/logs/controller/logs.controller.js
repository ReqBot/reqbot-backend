...
exports.create = function(req, res) {
   ...
   if(req.body.constructor === Object && Object.keys(req.body).length === 0){
        ...
        logs.create(new_logs, function(err, logs) {
            if (err)
            res.send(err);
            res.json({error:false,message:"Logs added successfully!",data:logs});
        });
    }
};
...
exports.update = function(req, res) {
    if(req.body.constructor === Object && Object.keys(req.body).length === 0){
        ...
        Logs.update(req.params.id, new Logs(req.body), function(err, logs) {
            if (err)
            res.send(err);
            res.json({ error:false, message: 'Logs successfully updated' });
        });
    }
...
exports.delete = function(req, res) {
    ...
    if (err)
    res.send(err);
    res.json({ error:false, message: 'Logs successfully deleted' });
  });
};